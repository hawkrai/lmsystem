...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
	...
	$templateCache.put('ng-table/pager.html', '<div class="ng-cloak ng-table-pager"><div class="dataTables_info" ng-show="!params.settings().hideDataTablesInfo" id="subjectList_info" style="font-size: 1.2em;"> Отображается {{(params.page()-1)*params.count()+1}}-{{(params.page()-1)*params.count()+params.data.length}} из {{params.total()}} записей </div><ul class="pagination"><li ng-repeat="page in pages" ng-class="{\'disabled\': !page.active && (page.type==\'prev\' || page.type==\'next\'), \'active\': !page.active && page.type !=\'prev\' && page.type !=\'next\' && page.type !=\'more\', \'previous\': page.type==\'prev\', \'next\': page.type==\'next\'}" ng-switch="page.type"><a ng-switch-when="more" ng-click="params.page(page.number)" href="">&#8230;</a><a ng-switch-when="prev" ng-click="params.page(page.number)" href="" title="Предыдущая "><span class="glyphicon glyphicon-chevron-left"></span></a><a ng-switch-when="next" ng-click="params.page(page.number)" href="" title="Следующая"><span class="glyphicon glyphicon-chevron-right"></span></a><a ng-switch-default ng-click="params.page(page.number)" href="" >{{page.number}}</span></a></li></ul><div ng-if="params.settings().counts.length" class="ng-table-counts btn-group pull-right"><button ng-repeat="count in params.settings().counts" type="button" ng-class="{\'active\':params.count()==count}" ng-click="params.count(count)" class="btn btn-default btn-xs"><span ng-bind="count"></span></button></div></div>');
}]);
    return app;
}));
